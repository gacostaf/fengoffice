...
class Notifier
{
	...
	static function notifyAction($object, $action, $log_data, $exclude_contacts_ids = null, $log_object = null)
	{
		...
		if (is_array($exclude_contacts_ids)) {
			foreach ($subscribers as $person) {
				if (!in_array($person->getId(), $exclude_contacts_ids))
					$tmp_subs[] = $person;
			}
			$subscribers = $tmp_subs;
		}
	...
	static function buildPeopleGroups($people, $object, $ignore_lang_and_timezone = false)
	{
		...
		if ($ignore_lang_and_timezone) {
			...
			foreach ($people as $user) {
				$key = "en_us|0";
				if (!isset($lang_groups[$key]))
					$lang_groups[$key] = array();
		...
		foreach ($lang_groups as $key => $users) {
			...
			$count = 0;
			foreach ($users as $u) {
				if (!isset($lang_group['groups'][$group_count]))
					$lang_group['groups'][$group_count] = array();
	...
	static function objectNotification($object, $people, $sender, $notification, $description = null, $descArgs = null, $properties = array(), $links = array())
	{
		...
		try {
			if (
				$object instanceof ProjectFile && ($object->getAttachToNotification() || $object->getFileType() && $object->getFileType()->getIsImage() && config_option('show images in document notifications')
								...
								if ($inv_user instanceof Contact) {
									if (can_access($inv_user, $object->getMembers(), ProjectEvents::instance()->getObjectTypeId(), ACCESS_LEVEL_READ)) {
										...
										if ($inv->getInvitationState() == 1)
											$state_desc = lang('yes');
										else if ($inv->getInvitationState() == 2)
											$state_desc = lang('no');
										else if ($inv->getInvitationState() == 3)
	...
	static function forgotPassword(Contact $user, $token = null)
	{
		...
		if (!$from_email)
			$from_email = owner_company()->getEmailAddress();
	...
	{
		if (
			(!$event instanceof ProjectEvent) || (!$invitation instanceof EventInvitation)
		...
		foreach ($people as $user) {
			...
			if ($event->getTypeId() != 2)
				$date .= " " . Localization::instance()->formatTime($event->getStart(), $tz_offset);
	 ...
	 */
	static function sendEmail($to, $from, $subject, $body = false, $type = 'text/html', $encoding = '8bit', $attachments = array(), $object_id = 0)
	{
		$ret = false;
		...
		foreach ($attachments as $a) {
			...
			if (array_var($a, 'cid'))
				$attach->setId(array_var($a, 'cid'));
			if (array_var($a, 'name'))
				$attach->setFilename(array_var($a, 'name'));
		...
		foreach ($emails as $email) {
			try {
				if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
					file_put_contents(CACHE_DIR . "/debug_notifications", "Start queued_email_id=" . $email->getId() . "\n", FILE_APPEND);
				...
				if ($fromSMTP && config_option("smtp_address")) {
					...
					$from_address = config_option("smtp_address");
					if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
						file_put_contents(CACHE_DIR . "/debug_notifications", "using config smtp address\n", FILE_APPEND);
					...
					$from_address = $sender_address;
					if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
						file_put_contents(CACHE_DIR . "/debug_notifications", "using user email in from address\n", FILE_APPEND);
				}
				...
				if (Plugins::instance()->isActivePlugin('mail') && config_option('use_mail_accounts_to_send_nots')) {
					...
					if ($ma instanceof MailAccount) {
						...
						$from = array($ma->getEmailAddress() => ($sender_name != "" ? $sender_name : $ma->getFromName()));
						if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
							file_put_contents(CACHE_DIR . "/debug_notifications", "using mail account in from address\n", FILE_APPEND);
				...
				if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
					file_put_contents(CACHE_DIR . "/debug_notifications", print_r(array('from' => $from, 'subject' => $subject), 1) . "\n", FILE_APPEND);
				...
				foreach ($from as $f_add => $f_name) {
					...
					$email->setFrom($fr);
					if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
						file_put_contents(CACHE_DIR . "/debug_notifications", "Real from: $fr\n", FILE_APPEND);
				}

				if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
					file_put_contents(CACHE_DIR . "/debug_notifications", print_r(array('to' => $to, 'cc' => $cc, 'bcc' => $bcc), 1) . "\n", FILE_APPEND);
				...
				if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
					file_put_contents(CACHE_DIR . "/debug_notifications", "End sending queued email " . $email->getId() . "\n", FILE_APPEND);

			} catch (Exception $e) {
				if (defined('DEBUG_NOTIFICATIONS') && DEBUG_NOTIFICATIONS)
					file_put_contents(CACHE_DIR . "/debug_notifications", "Error sending queued_email " . $email->getId() . "\n" . $e->getMessage() . "\n", FILE_APPEND);
				if ($result) {
					DB::rollback();
				}

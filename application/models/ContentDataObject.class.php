...
abstract class ContentDataObject extends ApplicationDataObject {
	...
	function validateObjMembersWithObjectRelatedMembers($member_ids, $is_drag_and_drop = false, $related_task_id = null) {
		...
		if (!in_array($objectName, ['timeslot', 'payment_receipt', 'expense'])) {
			return;
		}
		...
		if (is_numeric($related_task_id) && $related_task_id > 0) {
			...
			if($objectName == 'payment_receipt' || $objectName == 'expense') {
	...
	function canAssignTask(ProjectTask $task) {
		...
		} else if ($ot_name == 'payment_receipt' || $ot_name == 'expense') {
			/** @var PaymentReceipt $this */

...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
    ...
    protected $_userContentType = 'text/html';
        ...
        $this->_compoundLevelFilters = array(
            (self::LEVEL_ALTERNATIVE + self::LEVEL_RELATED) => array(
                self::LEVEL_ALTERNATIVE => array(
                    'text/html' => self::LEVEL_ALTERNATIVE,
                    'text/plain' => self::LEVEL_RELATED
                    )
                )
            );
    ...
    */
    protected function _fixHeaders()
    {
        if (count($this->_immediateChildren)) {
            // Si hay adjuntos, necesitamos el boundary (separador)
            $this->_setHeaderParameter('Content-Type', 'boundary', $this->getBoundary()); 
            $this->_headers->remove('Content-Transfer-Encoding');
        } else { 
            // Si es una notificacion normal, forzamos HTML puro */
            $this->_setHeaderParameter('Content-Type', 'boundary', null);
            $this->_setEncoding('8bit');
            //Forzamos content-type
            if ($this->getHeaders()->has('Content-Type')) { 
                $this->getHeaders()->get('Content-Type')->setValue('text/html'); 
                $this->getHeaders()->get('Content-Type')->setParameters(array('charset' => 'utf-8'));
            }
        }
